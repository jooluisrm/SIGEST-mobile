...
import { useProfessorsInfiniteQuery } from "../../../../src/api/professor";
...
export default function Professor() {
    ...
    const { 
        ...
        hasNextPage, 
        isFetchingNextPage 
    } = useProfessorsInfiniteQuery(debouncedSearchText);

    // Mapeamento dos resultados recebidos da API (data contém páginas no useInfiniteQuery)
    const professors = data?.pages.flatMap((page) => page.data || []) || [];
    ...
    const handleCardPress = (id: number) => {
        router.push(`/gerenciar/professor/${id}` as any);
    };
    ...
    return (
        <View style={styles.container}>
            ...
            {isLoading && professors.length === 0 ? (
                ...
                /* Lista de professores vindos da API com Rolagem Infinita */
                <FlatList
                    data={professors}
                    ...
                    renderItem={({ item }) => (
                        <ProfessorCard
                            ...
                            onPress={() => handleCardPress(item.id_professor)}
                        />
                    )}

...
import { CursoCard } from "@/components/gerenciar/curso/curso-card";
import { Ionicons } from "@expo/vector-icons";
import { useCoursesInfiniteQuery } from "../../../../src/api/curso";
...
export default function GerenciarCurso() {
    ...
    // Debounce de 500ms na busca para evitar requisições a cada tecla digitada
    useEffect(() => {
    ...
    const { 
        ...
        hasNextPage, 
        isFetchingNextPage 
    } = useCoursesInfiniteQuery(debouncedSearchText);
    ...
    useEffect(() => {
        if (error) {
            ...
            Alert.alert(
                "Erro de Conexão", 
                "Não foi possível buscar a lista de cursos. Verifique a conexão com a API."
            );
        }
    }, [error]);
    ...
    const handleAddPress = () => {
        router.push("/gerenciar/curso/cadastro");
    };

    const handleCardPress = (id: number) => {
        router.push(`/gerenciar/curso/${id}` as any);
    };
    ...
    return (
        <View style={styles.container}>
            ...
            {isLoading ? (
                ...
                /* Lista de cursos vindos da API com Rolagem Infinita */
                <FlatList
                    data={courses}
                    ...
                    renderItem={({ item }) => (
                        <CursoCard
                            name={item.name}
                            numberPeriods={item.number_periods}
